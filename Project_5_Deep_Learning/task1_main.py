"""
    ...
    Description: Build and  train a network to do digit
...
# class definitions: defined network Net with 2 convolution layers
class Net(nn.Module):
    def __init__(self):
...
# Plot the original images with plt
def plt_dataset(example_data, example_targets):
    fig = plt.figure()
    for i in range(6):
        plt.subplot(2, 3, i + 1)
        plt.tight_layout()
...
# Plot the tested result (prediction) with plt
def plt_test_dataset(input_data, input_targets):
    fig = plt.figure()
    for i in range(9):
        plt.subplot(3, 3, i + 1)
        plt.tight_layout()
        ...
        plt.title("Prediction: {}".format(
            input_targets.data.max(1, keepdim=True)[1][i].item()))
        plt.xticks([])
        plt.yticks([])
    plt.show()
...
# Load mnist from torchvision to ..torchvision/files/
def dataset_load(batch_size_train, batch_size_test):
...
# Test the model with different hyperparameters and print the result in console
...
# Plot the line graph of trained avg lose and test avg loss in blue and red
def print_loss(train_counter, train_losses, test_counter, test_losses):
    fig = plt.figure()
